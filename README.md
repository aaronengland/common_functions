...
The `get_ecdf` function takes an array of days between orders generated by `get_days_diff` and the days since the most recent order to return the probability of churn (i.e., ECDF).

Note: index 0 from the list returned by `get_days_diff` will be zero. This value should be dropped prior to calculating ECDF.

Arguments:
- `array`: array of days between orders.
- `number`: number of days since the most recent transaction date.
